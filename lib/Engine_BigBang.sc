...
Engine_BigBang : CroneEngine {
	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		SynthDef("jp2",{ | amp=0.75,note=40, mix=1.0, detune = 0.4,lpf=10,gate=1,timeScale=8 |
			...
			Out.ar(0,sig/7*EnvGen.ar(Env.perc(Rand(0.1,2),Rand(1,3),1,[4,-4]),timeScale:timeScale,doneAction:2)*amp);
		}).send(s);
		SynthDef("sine",{
			arg note,gate=1,timeScale=8;
